...
import siakreborn.matakuliahprasyarat.core.MataKuliahPrasyarat;
import siakreborn.matakuliahprasyarat.core.MataKuliahPrasyaratService;
import siakreborn.matakuliahprasyarat.core.MataKuliahPrasyaratServiceImpl;
...
public class RencanaStudiServiceImpl extends RencanaStudiServiceComponent {
  ...
  public List<HashMap<String, Object>> cekPrasyarat(UUID mahasiswaId) {
    List<HashMap<String, Object>> riwayatList = kelasMahasiswaService.getRiwayatKelasMahasiswa(mahasiswaId, "mataKuliah");
    HashMap<String, Object> riwayat = new HashMap<String, Object>();
    for (HashMap<String, Object> item : riwayatList) {
      String mataKuliahNama = (String) item.get("mataKuliahNama");
      if ( (Double) (item.get("nilaiAkhir")) >= 55) {
        riwayat.put(mataKuliahNama, "lulus");
      } else {
        riwayat.put(mataKuliahNama, "mengambil");
      }
    }
    ...
    for (RencanaStudi rencanaStudi : rencanaStudiList) {
      if (rencanaStudi.getSemester().getId().toString().equals(semester.getId().toString())) {
        ...
        for (KelasMahasiswa kelasMahasiswa : kelasMahasiswaList) {
          MataKuliah mataKuliah = kelasMahasiswa.getKelas().getMataKuliah();
          List<MataKuliahPrasyarat> prasyaratList = mataKuliahPrasyaratService.getAllMataKuliahPrasyaratFromParent(mataKuliah.getId());

          for (MataKuliahPrasyarat prasyarat : prasyaratList) {
            HashMap<String, Object> prasyaratMap = prasyarat.toHashMap();
            String nama = prasyarat.getMataKuliah().getNama();
            String syarat = prasyarat.getSyarat();

            String status = (String) riwayat.get(nama);

            if (status != null && ((syarat.equals("Sudah Lulus") && status.equals("lulus")) || syarat.equals("Sudah Mengambil"))) {
                prasyaratMap.put("status", "Memenuhi");
            } else {
              prasyaratMap.put("status", "Tidak Memenuhi");
            }
            
            result.add(prasyaratMap);
          }
        }
        return result;
      }

...
public class ObserverServiceImpl extends ObserverServiceDecorator {
  ...
  public List<Observer> filterObserver(UUID id) {
    List<Observer> observerList = getAllObserver();

    List<Observer> result = new ArrayList<>();
    for (Observer alumni : observerList) {
      if (((ObserverImpl) alumni).getMahasiswa().getProgramStudi().getId().equals(id)) {
        result.add(alumni);
      }
    }
    return result;
  ...
  public List<Observer> filterObserverPublic(UUID id) {
    List<Observer> publicList = getAllObserverPublic();
    List<Observer> filteredList = filterObserver(id);

    Set<UUID> publicIds = new HashSet<>();
      for (Observer alumni : publicList) {
          publicIds.add(alumni.getId());
    }

    List<Observer> result = new ArrayList<>();
    for (Observer alumni : filteredList) {
        if (publicIds.contains(alumni.getId())) {
            result.add(alumni);
        }
    }
  ...
  public Observer filterVerify(HashMap<String, Object> body) {
    String nama = (String) body.get("nama");
		String npm = (String) body.get("npm");
    String idStr = (String) body.get("programStudiId");
    UUID id = UUID.fromString(idStr);
    
    List<Observer> alumniList = filterObserver(id);
    Observer result = null;

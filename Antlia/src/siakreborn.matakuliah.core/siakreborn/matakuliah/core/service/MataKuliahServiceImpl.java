...
public class MataKuliahServiceImpl extends MataKuliahServiceComponent {
  ...
  public List<MataKuliah> filterMataKuliahByProgramStudi(UUID programStudiId) {
      ...
      for (Kurikulum kurikulum : kurikulumList) {
          mataKuliahList.addAll(mataKuliahRepository.getListObject("mata_kuliah_comp", "kurikulum_id", kurikulum.getId()));
      }
      return mataKuliahList;
  }

  public List<MataKuliah> filterMataKuliahByParentMataKuliah(UUID parentMataKuliahId) {
      MataKuliah mataKuliah = mataKuliahRepository.getObject(parentMataKuliahId);
      List<MataKuliah> mataKuliahList = mataKuliahRepository.getListObject("mata_kuliah_comp", "kurikulum_id", mataKuliah.getKurikulum());
      return mataKuliahList;
  }

  public List<MataKuliah> filterMataKuliahByKurikulum(UUID kurikulumId) {
    List<MataKuliah> mataKuliahList = mataKuliahRepository.getListObject("mata_kuliah_comp", "kurikulum_id", kurikulumId);
    return mataKuliahList;
  }
}

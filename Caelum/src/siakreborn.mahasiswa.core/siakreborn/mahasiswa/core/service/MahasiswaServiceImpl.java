...
public class MahasiswaServiceImpl extends MahasiswaServiceComponent {
  ...
  public List<Mahasiswa> filterMahasiswa(String columnName, UUID id) {
    List<Mahasiswa> mahasiswaList = mahasiswaRepository.getListObject("mahasiswa_impl", columnName, id);
    return mahasiswaList;
  }

  public List<Mahasiswa> filterMahasiswaPublic(String columnName, UUID id) {
    List<Mahasiswa> publicList = getAllMahasiswaPublic();
    List<Mahasiswa> filteredList = filterMahasiswa(columnName, id);

    Set<UUID> publicIds = new HashSet<>();
      for (Mahasiswa m : publicList) {
          publicIds.add(m.getId());
    }

    List<Mahasiswa> result = new ArrayList<>();
    for (Mahasiswa m : filteredList) {
        if (publicIds.contains(m.getId())) {
            result.add(m);
        }
    }
  ...
  public List<Mahasiswa> filterNotGraduated(String columnName, UUID id) {
    List<Mahasiswa> notGraduatedList = getNotGraduated();
    List<Mahasiswa> filteredList = filterMahasiswa(columnName, id);

    Set<UUID> notGraduatedId = new HashSet<>();
      for (Mahasiswa m : notGraduatedList) {
          notGraduatedId.add(m.getId());
    }

    List<Mahasiswa> result = new ArrayList<>();
    for (Mahasiswa m : filteredList) {
        if (notGraduatedId.contains(m.getId())) {
            result.add(m);
        }
    }
    return result;
  ...
  public Mahasiswa filterVerify(HashMap<String, Object> body) {
    String nama = (String) body.get("nama");
		String npm = (String) body.get("npm");
    String idStr = (String) body.get("programStudiId");
    UUID id = UUID.fromString(idStr);
    
    List<Mahasiswa> mahasiswaList = mahasiswaRepository.getListObject("mahasiswa_impl", "programstudi_id", id);
    Mahasiswa result = null;

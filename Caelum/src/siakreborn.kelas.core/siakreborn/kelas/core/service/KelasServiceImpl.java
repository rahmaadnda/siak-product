...
import siakreborn.matakuliah.core.MataKuliahServiceImpl;
...
public class KelasServiceImpl extends KelasServiceComponent {
  ...
  public List<HashMap<String, Object>> filterKelasByProgramStudi(UUID programStudiId) {
    ...
    for (Kurikulum kurikulum : kurikulumList) {
        mataKuliahList.addAll(mataKuliahService.filterMataKuliahByKurikulum(kurikulum.getId()));
    }
    
    List<Kelas> kelasList = new ArrayList<>();
    for (MataKuliah mataKuliah : mataKuliahList) {
        kelasList.addAll(kelasRepository.getListObject("kelas_comp", "matakuliah_id", mataKuliah.getId()));
    }
